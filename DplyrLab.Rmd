...
```{r}
my_flights %>% mutate(time_gained = dep_delay- arr_delay) %>% arrange(time_gained) %>% select(time_gained)
```
...
my_flights %>% count(origin) %>% arrange(n)
my_flights %>% count(dest) %>% arrange(-n)
my_flights$air_time
```
...
my_flights %>%
  ...
  na.omit() %>% 
  summarise(mean(air_time), max(air_time), min(air_time))

```
Consider flightselect()Consider flights coming into Houston (HOU and IAH). What is the mean arrival delay by departure airport? 
